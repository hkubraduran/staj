	...
	def send_at_com(self, com):
		if self.serial_connection:
			try:
				...
				time.sleep(5)
				response = self.serial_connection.read_all().decode()	#all data written to the serial port is read and the data is parsed by converting the byte array to string.
				return response
			except serial.SerialException as e:
